...
class NEO extends Coin{
    public static function readMinute($limit = 100){
        $url = "https://min-api.cryptocompare.com/data/histominute?fsym=NEO&tsym=USD&limit=".$limit;
        $result = Coin::read($url);
        return $result;
    }
    public static function readHour($limit = 100){
        $url = "https://min-api.cryptocompare.com/data/histohour?fsym=NEO&tsym=USD&limit=".$limit;
        $result = Coin::read($url);
        return $result;
    }
    public static function readDay($limit = 100){
        $url = "https://min-api.cryptocompare.com/data/histoday?fsym=NEO&tsym=USD&limit=".$limit;
        $result = Coin::read($url);
        return $result;
    }
}
